...
from thonny.plugins.classroom.adapters import GoAdapter, JavaScriptAdapter, LanguageAdapter, PythonAdapter
...
def bundled_adapters(user_data_dir: str | Path, root: Path | None = None) -> dict[str, LanguageAdapter]:
    root = root or application_root()
    runtimes = root / "runtimes"
    suffix = ".exe" if os.name == "nt" else ""
    python_exe = runtimes / "python" / f"python{suffix}"
    if not python_exe.exists():
        python_exe = Path(sys.executable)
    return {
        "python": PythonAdapter(python_exe),
        "javascript": JavaScriptAdapter(runtimes / "node" / f"node{suffix}"),
        "go": GoAdapter(runtimes / "go" / "bin" / f"go{suffix}", runtimes / "go",
